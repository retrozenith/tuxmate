...
export function isAurPackage(packageName: string): boolean {
  return (
    AUR_PATTERNS.some(pattern => packageName.endsWith(pattern)) ||
    KNOWN_AUR_PACKAGES.has(packageName)
  );
}
  ...
  const generatedCommand = useMemo(() => {
    ...
    if (selectedDistro === "arch" && aurPackageInfo.hasAur) {
      if (!hasYayInstalled) {
        ...
        const yayInstallCmd =
          "sudo pacman -S --needed git base-devel && git clone https://aur.archlinux.org/yay.git /tmp/yay && cd /tmp/yay && makepkg -si --noconfirm && cd - && rm -rf /tmp/yay";
