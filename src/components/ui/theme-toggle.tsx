  ...
  return (
    ...
    >
      <div className="flex justify-between items-center w-full">
        ...
        >
          {isDark ? (
            <Moon className="w-5 h-5 text-[var(--text-primary)]" strokeWidth={1.5} />
          ) : (
            <Sun className="w-5 h-5 text-[var(--text-primary)]" strokeWidth={1.5} />
          )}
        </div>
        <div
          className={cn(
            "flex justify-center items-center w-8 h-8 rounded-full transition-transform duration-300",
            isDark ? "bg-transparent" : "transform -translate-x-10"
          )}
        >
          {isDark ? (
            <Sun className="w-5 h-5 text-[var(--text-muted)]" strokeWidth={1.5} />
          ) : (
            <Moon className="w-5 h-5 text-[var(--text-muted)]" strokeWidth={1.5} />
          )}
        </div>
      </div>
    </div>
